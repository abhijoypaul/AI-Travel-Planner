...
const buildPrompt = ({ destination, startDate, endDate, travelers, budget, currency = 'INR', interests, travelStyle }) => {
  ...
  6. Choose searchable, real-world locations! Ensure every attraction, restaurant, and hotel exists and is searchable on Google Maps. If suggesting a small local restaurant, street food spot, or walking path that might not have a dedicated Google Maps listing, specify its address/location relative to the nearest prominent searchable landmark, street, or train station (for example, instead of just "Mama's Cafe", write "Mama's Cafe, near Shibuya Station" or "Scenic Walking Path near Ueno Park") so it can be successfully geocoded.
