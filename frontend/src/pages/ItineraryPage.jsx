...
export function ItineraryPage() {
  ...
  const renderTripDetails = (trip) => {
    ...
    return (
      <div className="wander-card overflow-hidden bg-white">
        {/* Trip header */}
        <div className="px-6 py-4 border-b border-slate-100 flex flex-col sm:flex-row items-start sm:items-center justify-between bg-slate-50/50 gap-3">
          <div>
            <h2 className="text-base font-bold text-slate-900">{trip.destination}</h2>
            <p className="text-xs text-slate-750 font-medium mt-0.5">
              {formatDate(trip.startDate)} – {formatDate(trip.endDate)} ·{" "}
              {trip.travelers} travelers
            </p>
          </div>
          <Link to={`/trip/${trip._id}`} className="w-full sm:w-auto">
            <button className="w-full sm:w-auto flex items-center justify-center gap-1.5 px-4 py-2 rounded-xl bg-indigo-50 text-indigo-750 text-xs font-bold hover:bg-indigo-100 transition-colors border border-indigo-100">
              View Full Trip
          ...
          {itinerary.map((day) => {
            ...
            return (
              <div key={day.day}>
                ...
                >
                  <div className="flex items-center gap-3">
                    <div className={`h-8 w-8 rounded-xl flex items-center justify-center flex-shrink-0 ${openDay === day.day ? "bg-indigo-600 shadow-sm" : "bg-slate-100"}`}>
                      <CalendarDays className={`h-4 w-4 ${openDay === day.day ? "text-white" : "text-slate-655"}`} />
                    </div>
                    <div className="text-left">
                      <span className="text-sm font-bold text-slate-900">Day {day.day}</span>
                      {day.date && (
                        <span className="text-xs text-slate-605 font-bold ml-2">· {day.date}</span>
                      )}
                    </div>
                ...
                {/* Day details */}
                {openDay === day.day && (
                  <div className="px-4 sm:px-6 pb-5 space-y-4 bg-slate-50/50 pt-2 border-t border-slate-100">
                    {/* Day Title */}
                    {day.title && (
      ...
      <div className="pt-4 pb-2 animate-fade-in-up">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
          ...
          <div className="lg:col-span-3 space-y-3">
            <p className="text-xs font-bold text-slate-750 uppercase tracking-wider px-1 mb-2">Select Trip</p>
            {trips.map((trip) => (
              <div key={trip._id} className="space-y-2.5">
                <button
                  ...
                  className={`w-full text-left px-4 py-3.5 rounded-xl border transition-all ${
                    selectedTrip?._id === trip._id
                      ...
                      : "bg-white border-slate-200 text-slate-705 hover:border-indigo-100"
                  }`}
                >
                  <div className="flex items-center gap-2.5">
                    <div className={`h-8 w-8 rounded-xl flex items-center justify-center flex-shrink-0 ${selectedTrip?._id === trip._id ? "bg-indigo-100" : "bg-slate-105"}`}>
                ...
                {/* Mobile Inline Details (only rendered under active selected trip button) */}
                {selectedTrip?._id === trip._id && (
                  <div className="block lg:hidden mt-2.5 mb-4 animate-fade-in">
          ...
          {/* Itinerary timeline (Desktop only) */}
